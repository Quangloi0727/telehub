var DFT = function ($) {
    ...
    var refreshArticles = function(resp){
        ...
        $('#paging').append(_.paging('product', resp.paging));
    ...
    /**
     * Tạo thẻ html hiển thị product
     * @param obj Dữ liệu product
     * @returns {*}
     */
    var newArticle = function(obj){
        if (obj.category == null) obj.category = [];
        return _.Tags([
            {tag: 'tr', attr: {class: 'product ' + obj.warning, 'data-id': obj._id, 'data-url': '/#product'}, childs: [
                ...
                {tag: 'td', attr: {class: 'task text-center'}, childs: [
                    {tag: 'a', content: obj.title, attr: {href: '/#product/' + obj._id}}
                ]},
                {tag: 'td', attr: {class: 'task text-justify'}, childs: [
                    {tag: 'a', content: obj.raw.length > 50 ? (obj.raw.substr(0, 50) + ' ... ') : obj.raw, attr: {href: '/#product/' + obj._id}}
                ]},
                ...
                {tag: 'td', attr: {class: 'text-center'}, childs: [
                    {tag: 'a', attr: {class: 'p-t-3 btn-flat-bg', href: "/#product/" + obj._id + '/edit', 'data-toggle': 'tooltip', 'data-placement': "top", 'data-original-title': "Sửa"}, childs: [
        ...
        async.waterfall([
            ...
            $.get(convertUrlForQuery(newUrl('articles', listFilter) + listSort + (_.has(window.location.obj, 'ticket-href') ? ('&' + window.location.obj['ticket-href']) : '')), function (resp) {
                ...
                if (resp.data.length == 0 && Object.keys(window.location.obj).length > 0) {
                    swal({
                        title: _config.MESSAGE.PRODUCT.SEARCH_NOT_FOUND_TITLE,
                        text: _config.MESSAGE.PRODUCT.SEARCH_NOT_FOUND_TEXT,
                        type: "warning",
    ...
    var bindClick = function () {
        ...
        $(document).on('click', '.btn-remove', function () {
            ...
            swal({
                    title: _config.MESSAGE.PRODUCT.CONFIRM_DELETE_PRODUCT,
                    text: _config.MESSAGE.PRODUCT.TEXT_CONFIRM_DELETE_PRODUCT,
                ...
                function () {
                    _AjaxObject('/articles/' + _id, 'DELETE', {}, function (resp) {
                        if (_.isEqual(resp.code, 200)) {
                            swal({title: 'Thành công', text: _config.MESSAGE.PRODUCT.TEXT_SUCCESS_DELETE_PRODUCT, type: "success"});
        ...
        $(document).on('click', '#btn-delSelection', function(){
            ...
            swal({
                    title: _config.MESSAGE.PRODUCT.CONFIRM_DELETE_MANY_PRODUCT,
                    text: _config.MESSAGE.PRODUCT.TEXT_CONFIRM_DELETE_MANY_PRODUCT,
                    type: "warning", showCancelButton: true, confirmButtonColor: "#DD6B55", confirmButtonText: "Có, chắc chắn !", closeOnConfirm: false
                },
                function () {
